...
class AddSegmentWarningConfigRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            'apply_time' => [
                function ($attribute, $value, $fail) use ($name, $type_id, $apply_from, $apply_to) {
                    if (BrandnameSegmentWarningConfig::where('name',$name)->where('type_id', $type_id)->whereDate('apply_from', '>=', $apply_from)->whereDate('apply_to', '<=', $apply_to)->count() > 0) {
                        $fail(trans('brandname.config.segment_warning_config.unique'));
                    }
                    if (BrandnameSegmentWarningConfig::where('name',$name)->where('type_id', $type_id)->whereDate('apply_from', '<=', $apply_from)->whereDate('apply_to', '>=', $apply_from)->count() > 0) {
                        $fail(trans('brandname.config.segment_warning_config.unique'));
                    }
                    if (BrandnameSegmentWarningConfig::where('name',$name)->where('type_id', $type_id)->whereDate('apply_from', '<=', $apply_to)->whereDate('apply_to', '>=', $apply_to)->count() > 0) {
                        $fail(trans('brandname.config.segment_warning_config.unique'));
                    }
                }
            ]
