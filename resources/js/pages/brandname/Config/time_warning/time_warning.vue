<template
    ><div>
        <portlet :title="$t('brandname.config.time_warning.list')">
            <data-table
                ref="table"
                url="/brandname/brandname-time-warning-config/listing"
                :columns="columns"
                :actions="actions"
                :order-column-index="12"
        ...
        <time-warning-config-modal
            ref="modal"
            :on-action-success="updateItemSuccess"
        ></time-warning-config-modal>
        <time-warning-config-history-modal
            ref="historyModal"
        ></time-warning-config-history-modal>
    </div>
</template>
...
import TimeWarningConfigModal from "./partials/TimeWarningConfigModal";
import TimeWarningConfigHistoryModal from "./partials/TimeWarningConfigHistoryModal";
export default {
    name: "TimeWarning",
    components: { TimeWarningConfigHistoryModal, TimeWarningConfigModal },
    middleware: "auth",
    head() {
        return {
            title: i18n.t("menu.brandname.configs.time_warning.title")
        };
    },
    meta: {
    ...
    methods: {
        ...
        active(table, rowData) {
            ...
            bootbox.confirm({
                ...
                message: this.$t(
                    "brandname.config.time_warning.notification.active"
                ),
                buttons: {
                    cancel: {
                ...
                callback: async function(result) {
                    if (result) {
                        let res = await axios.post(
                            "/brandname/brandname-time-warning-config/active",
        ...
        inactive(table, rowData) {
            ...
            bootbox.confirm({
                title: this.$t("alert.notice"),
                message: this.$t(
                    "brandname.config.time_warning.notification.inactive"
                ),
                buttons: {
                    cancel: {
                        label: this.$t("button.cancel")
                ...
                callback: async function(result) {
                    if (result) {
                        let res = await axios.post(
                            "/brandname/brandname-time-warning-config/inactive",
                            { id: rowData.id }
                        );
                        const { data } = res;
