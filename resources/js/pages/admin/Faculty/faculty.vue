    ...
    <div>
        <portlet :title="$t('admin.faculty.title')">
            ...
            <data-table
                    ref="table"
                    :columns="columns"
                    url="/admin/faculty/listing"
                    :fixed-columns-left="1"
                    :fixed-columns-right="1"
                    :actions="actions"
                    :search-placeholder="$t('admin.faculty.placeholder.search')"
...
<script>
    ...
    export default {
        name: "Faculty",
        components: {FacultyModal, Portlet},
        middleware: "auth",
        data() {
            return {};
        },
        computed: {
            columns() {
                return [
                    {
                        data: "name",
                        title: this.$t("admin.faculty.name")
                    },
                    {
                        data: "description",
                        title: this.$t("admin.faculty.description"),
                        orderable: false,
                        className: "wrap-text"
                    },
                    {
        ...
        methods: {
            ...
            async handleActive(table, rowData) {
                ...
                bootbox.confirm({
                    ...
                    callback: async function (result) {
                        if (result) {
                            let res = await axios.post("/admin/faculty/active", {
                                id: rowData.id
                            });
                            const {data} = res;
            ...
            async handleDisable(table, rowData) {
                ...
                bootbox.confirm({
                    ...
                    callback: async function (result) {
                        if (result) {
                            let res = await axios.post("/admin/faculty/disable", {
                                id: rowData.id
                            });
                            const {data} = res;
